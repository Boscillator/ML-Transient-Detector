...
def main():
    ...
    kernel_middle[kernel_size // 2] = 1.0
    ...
    fig, axs = plt.subplots(len(kernels) + 1, len(modes), figsize=(15, 10), sharex="col")
    ...
    for i, (kernel_name, kernel_results) in enumerate(results.items()):
        for j, mode in enumerate(modes):
            ...
            if kernel_name == "Start Impulse":
                ...
                axs[i+1, j].axvline(x=kernel_size//2, color='r', linestyle='--', alpha=0.5)
            elif kernel_name == "End Impulse":
                axs[i+1, j].axvline(x=kernel_size-1, color='r', linestyle='--', alpha=0.5)
